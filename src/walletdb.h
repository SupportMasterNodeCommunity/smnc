...
public:
    ...
    bool WriteZSMNCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSMNCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSMNCSeed_deprecated(uint256& seed);
    bool EraseZSMNCSeed();
    bool EraseZSMNCSeed_deprecated();

    bool WriteZSMNCCount(const uint32_t& nCount);
    bool ReadZSMNCCount(uint32_t& nCount);
