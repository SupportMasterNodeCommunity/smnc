...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSMNCStake(spend));
    } else {
        ...
        CSMNCStake* SMNCInput = new CSMNCStake();
        SMNCInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(SMNCInput);
    }
