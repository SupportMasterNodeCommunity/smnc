                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zsmnc ought to be enough for anybody." - Bill Gates, 2017
    ui->zsmncpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzsmncSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    // SupportMasterNodeCommunity settings
    QSettings settings;
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zsmncpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzsmnc_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zsmnc is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zsmnc, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zsmnc...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zsmnc in ") +
...
void PrivacyDialog::on_pushButtonSpendzsmnc_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zsmnc is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zsmnc, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zsmnc
        sendzsmnc();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zsmnc
    sendzsmnc();
}

void PrivacyDialog::on_pushButtonZSMNCControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZSMNCControlDialog* zSMNCControl = new ZSMNCControlDialog(this);
    zSMNCControl->setModel(walletModel);
    zSMNCControl->exec();
}

void PrivacyDialog::setZSMNCControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSMNCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzsmnc()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid SupportMasterNodeCommunity Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zsmncpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zsmnc is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SMNC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zsmnc</b>";
    ...
    // use mints from zsmnc selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSMNCControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZSMNCControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zsmnc require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zsmnc"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZSMNC_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zsmnc require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zsmnc"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zsmnc was spent successfully update the addressbook with the label
    ...
    // Clear zsmnc selector in case it was used
    ZSMNCControlDialog::setSelectedMints.clear();
    ui->labelzSMNCSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zsmnc Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " SMNC, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zsmnc Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zsmncTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zsmnc </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zsmnc "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zsmnc "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zsmnc "));
    ui->labelzsmncAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zsmnc </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zsmnc </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zsmnc </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzsmncSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzsmnc->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zsmnc
        ui->pushButtonMintzsmnc->setEnabled(false);
        ui->pushButtonMintzsmnc->setToolTip(tr("zsmnc is currently disabled due to maintenance."));

        // Spend zsmnc
        ui->pushButtonSpendzsmnc->setEnabled(false);
        ui->pushButtonSpendzsmnc->setToolTip(tr("zsmnc is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zsmnc
        ui->pushButtonMintzsmnc->setEnabled(true);
        ui->pushButtonMintzsmnc->setToolTip(tr("PrivacyDialog", "Enter an amount of SMNC to convert to zsmnc", 0));

        // Spend zsmnc
        ui->pushButtonSpendzsmnc->setEnabled(true);
        ui->pushButtonSpendzsmnc->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
