...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered smnc address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SMNC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal smnc:URI (smnc:XyZ...?message=example)
            {
...
static void NotifyzsmncReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzsmncReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzsmncReset.connect(boost::bind(NotifyzsmncReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzsmncReset.disconnect(boost::bind(NotifyzsmncReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zsmncTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
