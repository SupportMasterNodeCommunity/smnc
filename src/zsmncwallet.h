...
#ifndef SupportMasterNodeCommunity_ZSMNCWALLET_H
#define SupportMasterNodeCommunity_ZSMNCWALLET_H
...
public:
    CzsmncWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSMNC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSMNC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SupportMasterNodeCommunity_ZSMNCWALLET_H
