...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SMNC);
    ...
    if (!settings.contains("nAnonymizeSupportMasterNodeCommunityAmount"))
        settings.setValue("nAnonymizeSupportMasterNodeCommunityAmount", 1000);

    nAnonymizeSupportMasterNodeCommunityAmount = settings.value("nAnonymizeSupportMasterNodeCommunityAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSupportMasterNodeCommunityAmount"))
        SoftSetArg("-anonymizesmncamount", settings.value("nAnonymizeSupportMasterNodeCommunityAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in smnc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSupportMasterNodeCommunityAmount:
            return QVariant(nAnonymizeSupportMasterNodeCommunityAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSupportMasterNodeCommunityAmount:
            nAnonymizeSupportMasterNodeCommunityAmount = value.toInt();
            settings.setValue("nAnonymizeSupportMasterNodeCommunityAmount", nAnonymizeSupportMasterNodeCommunityAmount);
            emit anonymizeSupportMasterNodeCommunityAmountChanged(nAnonymizeSupportMasterNodeCommunityAmount);
            break;
        case CoinControlFeatures:
