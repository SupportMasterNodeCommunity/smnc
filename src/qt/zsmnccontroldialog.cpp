...
#include "ui_zsmnccontroldialog.h"
...
std::set<std::string> ZSMNCControlDialog::setSelectedMints;
std::set<CMintMeta> ZSMNCControlDialog::setMints;

ZSMNCControlDialog::ZSMNCControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSMNCControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSMNCControlDialog::~ZSMNCControlDialog()
{
    delete ui;
}

void ZSMNCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSMNCControlDialog::updateList()
{
...
void ZSMNCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSMNCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSMNC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSMNCControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSMNCControlDialog::GetSelectedMints()
{
...
void ZSMNCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
