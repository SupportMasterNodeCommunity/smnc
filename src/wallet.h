...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 SMNC at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zsmnc send
enum ZerocoinSpendStatus {
    ZSMNC_SPEND_OKAY = 0,                            // No error
    ZSMNC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSMNC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSMNC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSMNC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSMNC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSMNC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSMNC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSMNC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSMNC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSMNC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSMNC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSMNC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSMNC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSMNC_SPENT_USED_ZSMNC = 14,                      // Coin has already been spend
    ZSMNC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSMNC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZSMNCBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSMNCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzsmncAuto) const;
    ...
    */
    ...
    CzsmncWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzsmncTracker> zsmncTracker;
    ...
    void setZWallet(CzsmncWallet* zwallet)
    {
        zwalletMain = zwallet;
        zsmncTracker = std::unique_ptr<CzsmncTracker>(new CzsmncTracker(strWalletFile));
    }

    CzsmncWallet* getZWallet() { return zwalletMain; }
    ...
    /** zsmnc reset */
    boost::signals2::signal<void()> NotifyzsmncReset;
