...
CzsmncWallet::CzsmncWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsmnc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSMNCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSMNCSeed_deprecated()) {
                    LogPrintf("%s: Updated zsmnc seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsmnc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsmnc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzsmncWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSMNCCount(nCountLastUsed);
    else if (!walletdb.ReadZSMNCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzsmncWallet::Lock()
{
    seedMaster = 0;
}

void CzsmncWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzsmncWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSMNC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzsmncWallet::LoadMintPoolFromDB()
{
...
void CzsmncWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzsmncWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzsmncWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsmncTracker->HasPubcoinHash(pMint.first)) {
...
bool CzsmncWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSMNC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsmncTracker which also adds to database
    pwalletMain->zsmncTracker->Add(dMint, true);
...
void CzsmncWallet::SeedToZSMNC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzsmncWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzsmncWallet::GenerateDeterministicZSMNC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzsmncWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzsmncWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
